...
class ColumnExtractionWashing(object):
    def __init__(self):
        ...
        self.H2O_wahsing = self.df['value'].loc['H2O_washing']
        self.NaCl_conc = self.df['value'].loc['NaCl_conc']
        self.no_washing_cycles = self.df['value'].loc['No_washing_cycles']
        self.mass_NaCl = self.sorbent * self.NaCl_conc
        self.H2O_washing_total = self.H2O_wahsing * self.no_washing_cycles
